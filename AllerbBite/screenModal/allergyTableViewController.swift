...
//         self.navigationItem.rightBarButtonItem = self.editButtonItem
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        // #warning Incomplete implementation, return the numbe r of rows
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
                    ...
                    let allergy = Allergy[indexPath.row]
                    var context = cell.defaultContentConfiguration()
                    context.text = "\(allergy.symbol)  \(allergy.name)"
...
//    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Perform the action when the cell is clicked
            if let cell = tableView.cellForRow(at: indexPath) {
                if cell.accessoryView != nil {
                                // Cell is already selected, deselect it
                                cell.accessoryView = nil
                            } else {
                                // Cell is not selected, add the checkmark accessory
    ...
    */
    @IBAction func cancelButtonTapped(_ sender: UIBarButtonItem) {
        // Reset the selectedIndexPath variable to nil to deselect the currently selected row
        dismiss(animated: true, completion: nil)
        // Reload the table view to update the appearance of cells
